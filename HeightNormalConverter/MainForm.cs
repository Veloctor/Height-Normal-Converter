...
namespace HeightNormalConverter
{
	...
	public partial class MainForm : Form
	{
		...
		float NX(int x, int y)
			=> Marshal.ReadByte(normalMapData.Scan0 + (y * normalMapData.Stride) + x * bytesPerPixel + 2) / 127f - 1f;

		float NY(int x, int y)
			=> Marshal.ReadByte(normalMapData.Scan0 + (y * normalMapData.Stride) + x * bytesPerPixel + 1) / 127f - 1f;

		float NZ(int x, int y)
			=> Marshal.ReadByte(normalMapData.Scan0 + (y * normalMapData.Stride) + x * bytesPerPixel) / 127f - 1f;
